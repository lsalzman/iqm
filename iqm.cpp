...
bool parseobj(stream *f, const char *filename)
{
	...
	{ 
		...
		case 'm':
		{
			if (memcmp(c, "mtllib", 6) && !isspace(c[6])) continue;
			c += 7;
			while (isspace(*c)) c++;
			char *name = c;
			size_t namelen = strlen(name);
		...
		case 'u':
		{
			if (memcmp(c, "usemtl", 6) && !isspace(c[6])) continue;
			c += 6;
			while (isspace(*c)) c++;
			char *name = c;
			size_t namelen = strlen(name);
