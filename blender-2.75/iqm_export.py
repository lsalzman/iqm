...
def collectMeshes(context, bones, scale, matfun, useskel = True, usecol = False, filetype = 'IQM', ccwwinding = False):
    vertwarn = []
    ...
    for obj in objs:
        if obj.type == 'MESH':
            ...
            for face in data.tessfaces:
                ...
                # Quake winding is default CW (reversed)
                for i in range(2, len(faceverts)):
                    if ccwwinding:
                        mesh.tris.append((faceverts[i-1], faceverts[i], faceverts[0])) 
                    else:
...
def exportIQM(context, filename, usemesh = True, useskel = True, usebbox = True, usecol = False, scale = 1.0, animspecs = None, matfun = (lambda prefix, image: image), derigify = False, boneorder = None, ccwwinding = False):
    ...
    if filename.lower().endswith('.iqm'):
        ...
        return 'Unknown file type: ' + str(filename)

    print('Writing '+filetype+' with CCW winding: ' + str(ccwwinding))
    ...
    if boneorder:
        try:
            ...
            if len(names) != len(bones):
                ...
                return 'Bone order (' + str(len(names)) + ') does not match skeleton (' + str(len(bones)) + ')'
            print('Reordering bones')
            for bone in bones.values():
                bone.index = names.index(bone.name)
        except:
            print('Failed opening bone order: %s' % boneorder)
            return 'Failed opening bone order: ' + str(boneorder)

    bonelist = sorted(bones.values(), key = lambda bone: bone.index)
    if usemesh:
        meshes = collectMeshes(context, bones, scale, matfun, useskel, usecol, filetype, ccwwinding)
    else:
        meshes = []
    ...
    if filename:
        try:
            ...
            return 'Failed writing to ' + str(filename)
        if filetype == 'IQM':
        ...
        return 'No ' + str(filetype) + ' file was generated'
...
class ExportIQM(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    useccwwinding = bpy.props.BoolProperty(name="CCW winding", description="Counter clockwise winding", default=False)
    ...
    def execute(self, context):
        ...
        result = exportIQM(context, self.properties.filepath, self.properties.usemesh, self.properties.useskel, self.properties.usebbox, self.properties.usecol, self.properties.usescale, self.properties.animspec, matfun, self.properties.derigify, self.properties.boneorder, self.properties.useccwwinding)
        if not result:
            return {'FINISHED'}
        else:
